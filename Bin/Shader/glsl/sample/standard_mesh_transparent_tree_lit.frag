...
void main()
{
	...
	//Texture
    vec4 colorTexture = texture(texSampler0, fragTexCoord).xyzw;
	if (colorTexture.w <= 0.0) 
    {
		discard;
    }
    ...
    //Final Color
    color = colorLight * colorTexture.xyz * colorVertex;

	//Final Alpha
    float alpha = mat.lighting.y * colorTexture.w;
