...
void OpenGLES_011_Texturing::loadModel_Custom()
{
    for (int i = 0; i < g_CountLen; i++)
    {
        ...
        {
            String msg = "*********************** OpenGLES_011_Texturing::loadModel_Custom: Failed to load model: " + pModelObject->pathModel;
        ...
        {   
            String msg = "*********************** OpenGLES_011_Texturing::loadModel_Custom: Failed to load texture: " + pModelObject->pathTexture;
            F_LogError("%s", msg.c_str());
            throw std::runtime_error(msg.c_str());
        }
...
bool OpenGLES_011_Texturing::loadModel_VertexIndex(ModelObject* pModelObject, bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load 
    FMeshData meshData;
    ...
    {
        F_LogError("*********************** OpenGLES_011_Texturing::loadModel_VertexIndex load model failed: [%s] !", pModelObject->pathModel.c_str());
        return false; 
    }
    ...
    F_LogInfo("OpenGLES_011_Texturing::loadModel_VertexIndex: load model [%s] success, Vertex count: [%d], Index count: [%d] !", 
		...
		pModelObject->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGLES_011_Texturing::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGLES_011_Texturing::loadModel_VertexIndex: create buffer vertex failed: [%s] !", pModelObject->nameModel.c_str());
			return false;
		}
	}
...
bool OpenGLES_011_Texturing::loadModel_Texture(ModelObject* pModelObject)
{
    if (!pModelObject->pathTexture.empty())
    {
        ...
        F_LogInfo("OpenGLES_011_Texturing::loadModel_Texture: Load texture [%s] success !", pModelObject->pathTexture.c_str());
    }
...
void OpenGLES_011_Texturing::createShaderModules()
{
    ...
    for (int i = 0; i < g_ShaderCount; i++)
    {
        ...
        {
            String msg = "*********************** OpenGLES_011_Texturing::createShaderModules: Failed to create shader vertex: " + pathVert;
			F_LogError("%s", msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLESShaderModules.push_back(pShaderVertex);
        this->m_mapGLESShaderModules[pathVert] = pShaderVertex;
        F_LogInfo("OpenGLES_011_Texturing::createShaderModules: create shader [%s] success !", pathVert.c_str());
        ...
        {
            String msg = "*********************** OpenGLES_011_Texturing::createShaderModules: Failed to create shader fragment: " + pathFrag;
			F_LogError("%s", msg.c_str());
            throw std::runtime_error(msg);
        }
        this->m_aGLESShaderModules.push_back(pShaderFragment);
        this->m_mapGLESShaderModules[pathFrag] = pShaderFragment;
        F_LogInfo("OpenGLES_011_Texturing::createShaderModules: create shader [%s] success !", pathFrag.c_str());
    }
}
