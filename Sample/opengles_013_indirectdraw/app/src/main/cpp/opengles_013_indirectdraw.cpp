...
/////////////////////////// ModelMesh ///////////////////////////
bool OpenGLES_013_IndirectDraw::ModelMesh::LoadMesh(bool isFlipY, bool isTransformLocal, const FMatrix4& matTransformLocal)
{
    //1> Load
    ...
    if (this->typeMesh == F_Mesh_File)
    {
        ...
        {
            F_LogError("*********************** OpenGLES_013_IndirectDraw::ModelMesh::LoadMesh: load mesh failed: [%s] !", this->pathMesh.c_str());
            return false; 
        }
    }
    else if (this->typeMesh == F_Mesh_Geometry)
    {
        ...
        {
            F_LogError("*********************** OpenGLES_013_IndirectDraw::ModelMesh::LoadMesh: create geometry mesh failed: typeGeometry: [%s] !", F_GetMeshGeometryTypeName(this->typeGeometryType).c_str());
            return false; 
        }
    }
    else
    {
        F_Assert(false && "OpenGLES_013_IndirectDraw::ModelMesh::LoadMesh: Wrong typeMesh !")
        return false;
    }
    ...
    if (this->poTypeVertex == F_MeshVertex_Pos3Color4Normal3Tex2)
    {
        ...
        F_LogInfo("OpenGLES_013_IndirectDraw::ModelMesh::LoadMesh: load mesh [%s] success, [Pos3Color4Normal3Tex2]: Vertex count: [%d], Index count: [%d] !", 
                  this->nameMesh.c_str(),
    ...
    else if (this->poTypeVertex == F_MeshVertex_Pos3Color4Normal3Tangent3Tex2)
    {
        ...
        F_LogInfo("OpenGLES_013_IndirectDraw::ModelMesh::LoadMesh: load mesh [%s] success, [Pos3Color4Normal3Tangent3Tex2]: Vertex count: [%d], Index count: [%d] !", 
                  this->nameMesh.c_str(),
		...
		this->poIndexBuffer_Data != nullptr)
	{
		...
		{
			F_LogError("*********************** OpenGLES_013_IndirectDraw::loadModel_VertexIndex: create buffer vertex index failed: [%s] !", this->nameMesh.c_str());
			return false;
		}
	}
	else
	{
		...
		{
			F_LogError("*********************** OpenGLES_013_IndirectDraw::loadModel_VertexIndex: create buffer vertex failed: [%s] !", this->nameMesh.c_str());
			return false;
		}
	}
...
void OpenGLES_013_IndirectDraw::loadModel_Custom()
{
    ...
    for (int i = 0; i < g_ObjectCount; i++)
    {
        ...
        //Mesh
        {
            ...
            if (pMesh == nullptr)
            {
                F_LogError("*********************** OpenGLES_013_IndirectDraw::loadModel_Custom: can not find mesh: [%s] !", pModelObject->nameMesh.c_str());
            }
...
void OpenGLES_013_IndirectDraw::createGraphicsPipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            String msg = "*********************** OpenGLES_013_IndirectDraw::createGraphicsPipeline_Custom: Failed to create pipeline !";
            F_LogError("%s", msg.c_str());
...
void OpenGLES_013_IndirectDraw::createComputePipeline_Custom()
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        F_Assert(aShaderComps.size() == aDSLs.size() && "OpenGLES_013_IndirectDraw::createComputePipeline_Custom")
        ...
        for (size_t j = 0; j < count_cs; j++)
        {
            ...
            {
                String msg = "*********************** OpenGLES_013_IndirectDraw::createComputePipeline_Custom: Failed to create pipeline compute !";
                F_LogError("%s", msg.c_str());
                throw std::runtime_error(msg.c_str());
            }
...
OpenGLES_013_IndirectDraw::ModelMesh* OpenGLES_013_IndirectDraw::findMesh(const String& nameMesh)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		//Pipeline Graphics
		{
            ...
            for (int j = 0; j < count_ds; j++)
            {
                ...
                {
                    String msg = "*********************** OpenGLES_013_IndirectDraw::createDescriptorSets_Custom: Graphics: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
                    F_LogError("%s", msg.c_str());
                    throw std::runtime_error(msg.c_str());
                }
            }
        ...
        if (pModelObject->isUsedCompute)
		{
            ...
            for (int j = 0; j < count_comp; j++)
            {
                ...
                for (size_t p = 0; p < count_names; p++)
                {
                    ...
                    {
                        String msg = "*********************** OpenGLES_013_IndirectDraw::createDescriptorSets_Custom: Compute: Wrong DescriptorSetLayout type: " + nameDescriptorSet;
                        F_LogError("%s", msg.c_str());
                        throw std::runtime_error(msg.c_str());
                    }
                }
...
{   
    ...
    for (size_t i = 0; i < count; i++)
    {
		...
		{	
			F_Assert(false && "OpenGLES_013_IndirectDraw::drawMeshDefault_Custom")
		}
