#****************************************************************************
# LostPeterOpenGLES - Copyright (C) 2022 by LostPeter
# 
# Author:   LostPeter
# Time:     2024-12-02
# Github:   https://github.com/LostPeter/LostPeterOpenGLES
# Document: https://www.zhihu.com/people/lostpeter/posts
#
# This code is licensed under the MIT license (MIT) (http://opensource.org/licenses/MIT)
#****************************************************************************/

debug=${1}

./InstallRun_MacOS_Android_Sample.sh opengles_000_window $debug
./InstallRun_MacOS_Android_Sample.sh opengles_001_triangle $debug
./InstallRun_MacOS_Android_Sample.sh opengles_002_imgui $debug
./InstallRun_MacOS_Android_Sample.sh opengles_003_texture $debug
./InstallRun_MacOS_Android_Sample.sh opengles_004_model $debug
./InstallRun_MacOS_Android_Sample.sh opengles_005_camera $debug
./InstallRun_MacOS_Android_Sample.sh opengles_006_depth $debug
./InstallRun_MacOS_Android_Sample.sh opengles_007_stencil $debug
./InstallRun_MacOS_Android_Sample.sh opengles_008_blend $debug
./InstallRun_MacOS_Android_Sample.sh opengles_009_instancing $debug
./InstallRun_MacOS_Android_Sample.sh opengles_010_lighting $debug
./InstallRun_MacOS_Android_Sample.sh opengles_011_texturing $debug
./InstallRun_MacOS_Android_Sample.sh opengles_012_shadering $debug
./InstallRun_MacOS_Android_Sample.sh opengles_013_indirectdraw $debug
